...
impl TerraformManager {
    ...
    pub async fn initialize(&self) -> Result<()> {
        ...
        if tf_check.success() {
            ...
            return Err(TerraformError::BinaryNotFound {
                message: "Terraform not found or not executable. Make sure you have properly installed Terraform.".to_string()
            });
        }

        if !self.infrastructure_dir.exists() {
            return Err(TerraformError::EnvironmentNotFound {
                path: self.infrastructure_dir,
            });
        }
    ...
    fn create_workspace_config(
        ...
        if !env_dir.exists() {
            return Err(TerraformError::EnvironmentNotFound { path: env_dir });
        }
