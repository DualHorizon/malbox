...
pub async fn start_machine(label: String, snapshot: Option<String>) -> anyhow::Result<()> {
    ...
    let dom = Domain::lookup_by_name(&conn, &label)
    ...
    if let Some(snapshot) = snapshot {
        tracing::debug!("Found snapshot `{}` for domain `{}` in database, reverting..", snapshot, label);
